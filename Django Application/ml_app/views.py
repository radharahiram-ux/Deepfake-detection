...
class validation_dataset(Dataset):
    ...
    def __getitem__(self,idx):
        ...
        for i,frame in enumerate(self.frame_extract(video_path)):
            ...
            # Convert OpenCV BGR to RGB for standard neural network input
...
def index(request):
    if request.method == 'GET':
        ...
        if video_upload_form.is_valid():
            ...
            if allowed_video_file(video_file.name) == False and allowed_image_file(video_file.name) == False:
                video_upload_form.add_error("upload_video_file","Only video or image files are allowed ")
                return render(request, index_template_name, {"form": video_upload_form})
            
            if allowed_image_file(video_file.name):
                # Handle as image
                saved_image_file = 'uploaded_image_'+str(int(time.time()))+"."+video_file_ext
                image_path = os.path.join(settings.PROJECT_DIR, 'uploaded_images', saved_image_file)
                with open(image_path, 'wb') as vFile:
                    shutil.copyfileobj(video_file, vFile)
                request.session['image_file_name'] = image_path
...
image_index_template_name = 'image_index.html'
image_predict_template_name = 'image_predict.html'

ALLOWED_IMAGE_EXTENSIONS = set(['jpg', 'jpeg', 'png', 'bmp', 'tiff', 'webp', 'gif'])
...
def image_index(request):
    if request.method == 'GET':
        ...
        if 'image_file_name' in request.session:
            del request.session['image_file_name']
        return render(request, image_index_template_name, {"form": image_upload_form})
    else:
        ...
        if image_upload_form.is_valid():
            ...
            if image_file.size > int(settings.MAX_UPLOAD_SIZE):
                image_upload_form.add_error("upload_image_file", "Maximum file size 100 MB")
                return render(request, image_index_template_name, {"form": image_upload_form})

            if not allowed_image_file(image_file.name):
                image_upload_form.add_error("upload_image_file", "Only image files (jpg, png, bmp, etc.) are allowed.")
                return render(request, image_index_template_name, {"form": image_upload_form})
...
def image_predict_page(request):
    if request.method == "GET":
        if 'image_file_name' not in request.session:
            return redirect("ml_app:image_home")
        ...
        try:
            ...
            if img is None:
                return render(request, image_predict_template_name, {"error": "Could not read the uploaded image."})

            rgb_img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

            # Save the original uploaded image for display
            original_display_name = image_file_name
            
            # Face detection
            face_locations = face_recognition.face_locations(rgb_img)
            
            cropped_face_name = None
            if len(face_locations) > 0:
                padding = 40
                top, right, bottom, left = face_locations[0]
                top = max(0, top - padding)
                ...
                face_img = pImage.fromarray(rgb_face, 'RGB')
                cropped_face_name = image_file_name_only + "_face_cropped.png"
                face_path = os.path.join(settings.PROJECT_DIR, 'uploaded_images', cropped_face_name)
                face_img.save(face_path)

            # Prepare image for model prediction
            # The model expects a sequence of frames; we'll duplicate the single image
            sequence_length = 20  # default sequence length for image prediction

            frame = rgb_img
            if len(face_locations) > 0:
                top, right, bottom, left = face_locations[0]
                frame = rgb_img[max(0,top-40):min(rgb_img.shape[0],bottom+40), max(0,left-40):min(rgb_img.shape[1],right+40), :]

            # Apply transforms and create a sequence by duplicating the frame
            transformed_frame = train_transforms(frame)
            frames_list = [transformed_frame] * sequence_length
            ...
            if not model_path:
                return render(request, image_predict_template_name, {
                    "error": "No model found for prediction. Please add trained models to the models directory.",
                    "original_image": original_display_name,
                    "cropped_face_image": cropped_face_name,
                })
            ...
            path_to_model = os.path.join(settings.PROJECT_DIR, model_path)
            model.load_state_dict(torch.load(path_to_model, map_location=torch.device('cpu')))
            model.eval()

            # Run prediction
            prediction = predict(model, frames_tensor, './', image_file_name_only)
            confidence = round(prediction[1], 1)
            output = "REAL" if prediction[0] == 0 else "FAKE"
